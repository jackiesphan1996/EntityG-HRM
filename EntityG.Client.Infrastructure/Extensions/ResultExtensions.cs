...
namespace EntityG.Client.Infrastructure.Extensions
{
    public static class ResultExtensions
    {
        ...
        public static async Task<PagingResult<T>> ToPagingResult<T>(this HttpResponseMessage response)
        {
            var responseAsString = await response.Content.ReadAsStringAsync();
            var responseObject = JsonSerializer.Deserialize<PagingResult<T>>(responseAsString, new JsonSerializerOptions
            {
                PropertyNameCaseInsensitive = true
            });
            return responseObject;
